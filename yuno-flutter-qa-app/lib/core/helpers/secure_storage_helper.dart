...
class SecureStorage {
  ...
  Future<List<Credential>> getCredentials() async {
    await storage.reload();
    final jsonString = storage.getString('saved_credentials') ?? '[]';
    try {
  ...
  Future<void> setCurrentCredential(Credential credential) async {
...
final credentialsListNotifier = FutureProvider<List<Credential>>(
  (ref) async {
    return await ref.watch(providerStorage).getCredentials();
  },
);
